...
object APIExample extends App {
  ...
  val zeros: Tensor[Float] =
    Tensor.zeros[Float](Shape(2, 2, 9, 6)) // equivalent to tf.zeros((2, 2, 9, 6), dtype=tf.float32)
  ...
  val matrix = Tensor[Float](
    Tensor(1, 2, 3),
    Tensor(1, 2, 3),
    Tensor(1, 2, 3)
  ) // equivalent to tf.tensor([1,2,3], [1, 2, 3], [1, 2, 3], dtype=tf.float32)
  // Operations:
  ...
  val gradientsAndVariables = tf.train
    .GradientDescent(1e-6f)
